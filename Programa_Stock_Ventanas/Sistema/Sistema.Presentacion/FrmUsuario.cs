...
namespace Sistema.Presentacion
{
    public partial class FrmUsuario : Form
    {
        ...
        private void Listar()
        {
            try
            {
                DgvListado.DataSource = UsuarioSN.Listar();
                this.Formato();
                this.Limpiar();
        ...
        private void Buscar()
        {
            try
            {
                DgvListado.DataSource = UsuarioSN.Buscar(TxtBuscar.Text);
                this.Formato();
                LblTotal.Text = "Total registros: " + Convert.ToString(DgvListado.Rows.Count);
            }
            catch (Exception ex)
        ...
        private void CargarRol()
        {
            try
            {
                CboRol.DataSource = RolSN.Listar();
        ...
        private void BtnInsertar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Rpta = UsuarioSN.Insertar(Convert.ToInt32(CboRol.SelectedValue), TxtNombre.Text.Trim(), Convert.ToString(CboTipoDocumento.Text), TxtNumDocumento.Text.Trim(), TxtDireccion.Text.Trim(), TxtTelefono.Text.Trim(), TxtEmail.Text.Trim(), TxtClave.Text.Trim());
        ...
        private void BtnActualizar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Rpta = UsuarioSN.Actualizar(Convert.ToInt32(TxtId.Text), Convert.ToInt32(CboRol.SelectedValue), TxtNombre.Text.Trim(), Convert.ToString(CboTipoDocumento.Text), TxtNumDocumento.Text.Trim(), TxtDireccion.Text.Trim(), TxtTelefono.Text.Trim(), this.EmailAnt, TxtEmail.Text.Trim(), TxtClave.Text.Trim()); ; ;
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in DgvListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Rpta = UsuarioSN.Eliminar(Codigo);
            ...
            try
            {
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in DgvListado.Rows)
                    {
                        ...
                        {
                            Codigo = Convert.ToInt32(row.Cells[1].Value);
                            Rpta = UsuarioSN.Desactivar(Codigo);
